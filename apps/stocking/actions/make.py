...
def make_xy(ds: pd.DataFrame,
    ...
    if return_Xy:
        ...
        return x, y, x_latest, y_latest
    if to_numpy:
        return pd.concat([x, y],
                         axis=1).to_numpy(), x_latest.to_numpy(), y_latest.to_numpy()
    return pd.concat([x, y], axis=1), x_latest, y_latest
