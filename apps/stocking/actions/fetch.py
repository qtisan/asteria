...
import pandas as pd
import requests
from utils.functional import get_file_modify_time, same_day, is_yesterday
import os
import io

from apps.stocking.metainfo import logger, CACHED_ROOT
...
#          date  close   high    low   open   yest  turnover    volume        amount         total        circul
...
def read_cached_data(code: str):
    ...
    if (same_day(now, mtime) and
        ((now.hour > threshold and mtime.hour > threshold) or
         (now.hour < threshold and mtime.hour < threshold))) or (is_yesterday(
             now, mtime) and mtime.hour > threshold and now.hour < threshold):
...
def fetch(code: str, *args, **kwargs):
    logger.debug('===============')
    logger.debug('[{0}] - Fetching...'.format(code))
    logger.debug('===============')
